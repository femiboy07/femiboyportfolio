        ...
        const handleFetchProjects = async () => {
            try {
                ...
                const findData = data.find(
                    (item) => item.projectName?.trim().toLowerCase() === param?.slug?.toString().toLowerCase()
                );
        ...
        <main className="min-[651px]:text-[.9rem] h-full max-w-[1400px] max-[650px]:px-[8.9vw]">
            ...
            <div className="flex flex-col relative mb-5">
                <div className="w-full relative overflow-hidden">
                    <div className="h-full  visible flex justify-center flex-row" style={{ backfaceVisibility: 'hidden' }}>
                        <div className="min-h-[40vh]  max-h-[42.5vw] relative flex-grow-0  flex-shrink-0 outline-none " style={{ flexBasis: "inherit" }}>
                            <Image src={`/${currentProject?.projectImage}`} sizes="(max-width:580px) 100vw ,(min-width:581px) 50vw " style={{ objectFit: 'cover' }} width={500} height={500} alt="project-image" className="w-auto h-auto block max-w-full" />
                        </div>
                    </div>
                </div>
            </div>
